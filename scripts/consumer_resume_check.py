#!/usr/bin/env python3
"""
consumer_resume_check.py - the consumer-side `resume`'s two already-scripted, notify-only
hub-staleness checks, chained into one call: a connected project's own `resume` only ever needs
to ask "has the hub toolkit\\ I import from fallen behind its own upstream," not the hub's own
